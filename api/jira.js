...
// Busca el id de un campo por su nombre (sin tildes, sin distinguir mayúsculas)
const _cfPorNombre = {};
async function idCampoPorNombre(auth, cloud, nombre) {
  const clave = nombre.toLowerCase();
  if (_cfPorNombre[clave] !== undefined) return _cfPorNombre[clave];
  const norm = t => (t || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .replace(/\s+/g, ' ').trim().toLowerCase();
  ...
  try {
    ...
    if (r.status === 200 && Array.isArray(r.body)) {
      const hit = r.body.find(f => norm(f.name) === objetivo);
      _cfPorNombre[clave] = hit ? hit.id : null;
    }
  } catch (e) { _cfPorNombre[clave] = null; }
  return _cfPorNombre[clave] || null;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (type === 'ventas') {
      ...
      const STORY_FIELDS = ['summary','status','assignee','parent','customfield_10015','duedate','subtasks','customfield_10725','customfield_10726','issuetype','customfield_10930','customfield_11070','customfield_11004','customfield_10895','customfield_10928','customfield_10929','customfield_10934','story_points','customfield_10016','timespent'];
      ...
      // El estado y las fechas son los de la propia historia.
      let CF_ENT = await idCampoEntregable(auth, JIRA_CLOUD);
      const CF_INI = await idCampoPorNombre(auth, JIRA_CLOUD, 'Fecha inicio desarrollo');
      ...
      // Solo historias marcadas como entregable (se excluyen las épicas)
      items = items.filter(it => {
        const t = it.fields.issuetype || {};
        if (t.hierarchyLevel === 1 || t.name === 'Epic') return false;
        return CF_ENT ? esEntregable(it.fields[CF_ENT]) === true : false;
      });
      ...
      items.forEach(it => {
        ...
        it.fields._epica = epKey
          ...
          : null;
        // Fechas de desarrollo; si el campo no existe se cae a las fechas estándar
        it.fields._ini = (CF_INI ? it.fields[CF_INI] : null) || it.fields.customfield_10015 || null;
